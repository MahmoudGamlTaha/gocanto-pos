...
class sales extends Secure_area
{
    ...
    public function index($force = '')
    {
        ...
        $data_re = [
            ...
            'location'             => '',
            'overall_summary_data' => $model->getSummaryData(['start_date' => $data_report['start_date'], 'end_date' => $data_report['end_date'], 'sale_type' => $force]),
            'export_excel'         => 0,
            'sale_type'            => $force,
            'last'                 => true,
    ...
    public function get_ajax_sale_details()
    {
        ...
        echo json_encode(['total' => $total, 'subtotal' => $subtotal, 'taxes' => $taxes, 'due' => $amountdue]);
    }
    ...
    public function add_payment($payment_type = null, $payment_amount = null)
    {
    ...
    public function add($item_id_or_number_or_item_kit_or_receipt = null, $service_id = null, $noreload = null)
    {
        $data = [];
    ...
    public function edit_item($line, $ajax = false)
    {
        $data = [];
        ...
        switch ($this->sale_lib->get_mode()) {
            ...
            case 'shipping':
                ...
                if (!$customer) {
                    ...
                    if (isset($customer_data['person_id'])) {
    ...
    public function save($sale_id)
    {
        ...
        if ($this->Sale->update($sale_data, $sale_id)) {
            echo json_encode(['success' => true, 'message' => $this->lang->line('sales_successfully_updated')]);
        } else {
            echo json_encode(['success' => false, 'message' => $this->lang->line('sales_unsuccessfully_updated')]);
        }
    }

    public function _payments_cover_total()
    {
        $total_payments = 0;
    ...
    public function _reload($data = [])
    {
        if ($this->sale_lib->get_mode() === 'shipping') {
            redirect('sales/index/shipping');
        } else {
            redirect('sales');
        }
    }
    ...
    public function suspend()
    {
        ...
        $this->sale_lib->clear_all();
        $this->_reload(['success' => $this->lang->line('sales_successfully_suspended_sale')]);
    }
    ...
    public function pending_orders_to_shipping($id_order)
    {
        $this->load->model('Order');
        $data['location'] = $this->Order->get_info($id_order);
        $data['location'] = $data['location']['info']->row()->location;
        if ($data['location'] == $this->session->userdata('dblocation')) {
            redirect('reports/pending_orders/'.$id_order.'/2');
        } else {
