...
class Chat_model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        $this->logged = ((bool) $this->session->userdata('person_id'));
        $this->getCon();
        if ($this->logged) {
            $this->getUser();
        }
        ...
        if (!isset($_SESSION['chat'][$this->user->chat_id])) {
            $_SESSION['chat'][$this->user->chat_id] = [];
        }
    }

    // --------------------------------------------------------------------

    public function cleanTyping($id = false)
    {
        if (!$id) {
    ...
    public function disableUser($disabled = false)
    {
        $id = $this->user->chat_id;
    ...
    public function isDisabledUser($id = false, $disabled = false)
    {
        if (!$id) {
            return false;
        }
    ...
    public function getChatUsers($update = false)
    {
        $this->updateUsers();
        if ($update) {
    ...
    public function getCon($conditions = [], $fields = '')
    {
        if ($this->con) {
            return $this->con;
        }
    ...
    public function getUsers($conditions = [], $fields = '')
    {
        $this->updateLoggedUser();
        $this->updateUsers();
    ...
    public function setChatId($id, $location)
    {
        if (!$id || !$location) {
            return false;
        }
    ...
    public function setStatus($chat_id = false, $status = 1)
    {
        if (!$chat_id) {
            return false;
        }
    ...
    public function setTyping($to = false, $status = true)
    {
        if (!$to) {
            return false;
        }
