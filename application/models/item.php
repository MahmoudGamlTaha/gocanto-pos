...
class item extends CI_Model
{
    ...
    public function get_all($data = [], $limit = 10000, $offset = 0)
    {
        $this->con->from('items');
        if (count($data) > 0 && isset($data['low_inventory'])) {
            $this->get_filt($data);
        }
    ...
    */
    ...
    public function get_info($item_id, $campos = false)
    {
        if ($campos) {
            $this->con->select($campos);
        }
        $this->con->from('items');
    ...
    */
    ...
    public function get_multiple_info($item_ids, $datos = false)
    {
        if ($datos) {
            $this->con->select($datos);
        }
        $this->con->from('items');
    ...
    */
    ...
    public function save_in_other_inventory(&$item_data, $item_id = false, $db = null)
    {
        if ($db != '...' && $db != null) {
    ...
    */
    ...
    public function delete($item_id)
    {
        ...
        return $this->con->update('items', ['deleted' => 1]);
    }
    ...
    */
    ...
    public function suggest2($search = '', $is_ser = false)
    {
        $this->con->from('items');
        $this->con->where('deleted', 0);
        if ($is_ser) {
            $this->con->where('item_id >', 5);
        } else {
    ...
    public function get_item_search_suggestions($search, $limit = 25)
    {
        $suggestions = [];
    ...
    */
    ...
    public function report_broken($item_id = -1)
    {
        if ($item_id >= 0) {
