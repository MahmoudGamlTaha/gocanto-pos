...
class customers extends Person_controller
{
    ...
    public function account($customer_id = -1, $status = 'pay')
    {
    ...
    */
    ...
    public function exists_email($email = '')
    {
        ...
        if ($this->Customer->exists_email($email) != '0') {
            //si existe
            echo json_encode(['success' => true, 'message' => 'existe', 'email' => $this->Customer->exists_email($email)]);
        } else {
            //no existe
            echo json_encode(['success' => false, 'message' => 'no existe', 'email' => $this->Customer->exists_email($email)]);
        }
    }
    ...
    */
    ...
    public function save($customer_id = -1)
    {
        ...
        if ($this->Customer->save($person_data, $customer_data, $customer_id)) {
            //New customer
            if ($customer_id == -1) {
                echo json_encode(['success' => true, 'message' => $this->lang->line('customers_successful_adding').' '.
                $person_data['first_name'].' '.$person_data['last_name'], 'person_id' => $person_data['person_id'], ]);
            } else {
                //previous customer

                echo json_encode(['success' => true, 'message' => $this->lang->line('customers_successful_updating').' '.
                $person_data['first_name'].' '.$person_data['last_name'], 'person_id' => $customer_id, 'retorno' => $person_data, ]);
            }
        } else {
            //failure

            echo json_encode(['success' => false, 'message' => $this->lang->line('customers_error_adding_updating').' '.
    ...
    */
    ...
    public function save_credit($person_id = -1, $type = 'pay')
    {
