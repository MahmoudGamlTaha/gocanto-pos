...
class service extends CI_Model
{
    ...
    {
        $this->con->from('model');
        $this->con->where(['model_name' => $model_id, 'brand_id' => $brand_id]);
        $this->con->limit(1);
        $query = $this->con->get();
        if ($query->num_rows() == 1) {
    ...
    public function save($service_data, $service_id = -1)
    {
        if (!$this->exists($service_id)) {
            ...
            if (!$brand_id) {
                $brand_id = $this->save_brand(['brand_name' => $service_data['brand_id']]);
            }
            $model_id = $this->exists_model($service_data['model_id'], $brand_id);
            if (!$model_id) {
                $model_id = $this->save_model(['model_name' => $service_data['model_id'], 'brand_id' => $brand_id]);
            }
    ...
    public function save_items($service_id = false, $items = [])
    {
        if ($service_id && $this->exists($service_id) && is_array($items) && count($items) > 0) {
            foreach ($items as $item) {
                $this->con->insert('service_items', ['service_id' => $service_id, 'item_id' => $item['item'], 'unit_price' => $item['preci'], 'is_Kit' => $item['kit']]);
            }
        } else {
            return false;
        }
    ...
    public function get_all($data = [], $limit = 5000, $offset = 5)
    {
    ...
    public function get_info($service_id = 0)
    {
        ...
        if ($query->num_rows() == 1) {
            ...
            return (Object) ['service_id' => -1, 'first_name' => '', 'last_name' => '', 'serial' => '', 'brand_name' => '', 'add_pay' => '', 'status' => '', 'price' => '', 'model_name' => '', 'problem' => '', 'person_id' => '', 'items' => []];
        }
    }
    ...
    public function search($service_id, $term, $limit = 5000, $offset = 5)
    {
        ...
        $by_id = $by_id ? $by_id->result() : [];
        $by_term = $by_term ? $by_term->result() : [];
    ...
    public function suggest2($search = '', $without = false)
    {
        ...
        if ($without) {
            $this->con->where('service_id !=', $without);
        }
        $query = $this->con->get();
    ...
    public function suggest_model($search = '', $brand = '')
    {
        ...
        if ($brand != '') {
            $this->con->where('brand_name', $brand);
        }
