<?php 
if (!$referen) {
    echo form_open('backup/confirm/-1', ['id' => 'form-back']) ?>
<div>
	<div id="login_form">
    ...
    ?>
		...
		<div class="field_row clearfix" id="msg_after_cong" style="display: none">	
			<?php echo form_label($this->lang->line('config_wait_back'), 'language', ['class' => 'lable-form', 'style' => 'width:200px;']);
    ?>
		</div>
...
<?php 
    ...
    ?>
<?php echo form_open_multipart('backup/confirm/-7', ['id' => 'recover-form']);
    ?>
<div>	
	<div class="field_row clearfix" style="margin: 0 0 5px 0">
		<div>
    ...
    ?>
	<div class="field_row clearfix" style="margin: 0 0 5px 0">
		<div id="radio" style="margin-top: 10px;"><input type="checkbox"  name="recover-backup" style="margin-right: 10px;"><?php echo $this->lang->line('config_recover_f_backup');
    ?></div>
	</div>
	<div id="option-backup" class="field_row clearfix" style="margin: 0 0 5px 0">
		<div class="sub-title-view"> <?php echo $this->lang->line('config_backup_info');
    ?> </div>
	</div>
	<div id="list-backup" class="field_row clearfix" style="margin: 0 0 5px 0">
		<div class="field_row clearfix" style="margin: 0 0 5px 0">
			<label style="float: none;"><?php echo $this->lang->line('config_backup_list').':';
    ?></label>
    ...
    foreach ($files as $key) {
        if (strpos($key, $bd) !== false) {
            ...
            ?>	
						<li><input type="radio" name="list-back" value="<?php echo $key?>" <?php echo $a == 0 ? 'checked=checked' : '';
            $a++;
            ?> ><?php echo $key;
            ?></li>
	...
	<div id="file-ext" class="field_row clearfix" style="<?php echo $display ?>margin: 0 0 5px 0">
		<div style="width: 100%; float: left; margin-top: 5px">
			...
			<?php echo form_label($this->lang->line('config_recover_fe').':', 'name', ['class' => 'lable-form-required']);
    ?>
			<div class='form_field' style="-webkit-border-radius: 5px; -moz-border-radius: 5px; border-radius: 5px; ">
				...
				<div class="upload_label"> <?php echo $this->lang->line('config_recover_f');
    ?> </div>
			</div>
			</div>
		</div>
	...
	$('#dow').click(function() {
		if (!$('#radio input').is(':checked')){
			...
			if (filename.indexOf('[')!=-1){
				filename=filename.replace('[<?php echo $this->lang->line('config_backup_auto_file');
    ?>]',"")+"/1";
			}
