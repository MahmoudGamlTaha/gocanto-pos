...
class order extends CI_Model
{
    ...
    public function save($order_data, $order_items_data)
    {
        if ($this->con->insert('orders', $order_data)) {
            ...
            foreach ($order_items_data as $item) {
                $this->con->insert('order_items', array_merge($item, ['id_order' => $order_id]));
            }
    ...
    public function cancel($order_id)
    {
        ...
        return $this->con->update('orders', ['status' => 2]);
    }

    public function complete($order_id = 0, $sale_id = 0)
    {
        $order_data = ['status' => 1, 'sale_id' => $sale_id];
        $this->con->where('id', $order_id);
    ...
    public function get_all($data, $limit = 5000, $offset = 0)
    {
        $this->con->from('orders');
        ...
        if (isset($data['filter_location']) && $data['filter_location'] != '') {
            $this->con->where('location', $data['filter_location']);
        } else {
            $this->con->where('location !=', $this->session->userdata('dblocation'));
        }
    ...
    public function check_availability($order_id)
    {
        $con = 0;
        $ctotal = 0;
        $string = '';
