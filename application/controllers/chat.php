...
class chat extends CI_Controller
{
    ...
    {
        if (is_array($json)) {
            $json['online'] = $this->is_online();
        }
        $this->load->view('partial/json', ['json' => $json]);
    }
    ...
    public function chatheartbeat()
    {
        ...
        $query = $this->con->update('chat', ['recd' => 1]);
        ...
        $data = [
            'ty'    => $typing,
            'items' => $items,
            'query' => isset($_SESSION['chat'][$this->user->chat_id]['history']) ? $_SESSION['chat'][$this->user->chat_id]['history'] : [],
        ];
        $this->putSession($session);
        $this->print_json($data);
    }
    ...
    public function friendslist()
    {
        ...
        if (!$this->chat_model->logged) {
            $this->print_json([]);
        }
        $list = $this->chat_model->getChatUsers($update);
        $data = [
            'a' => 1,
            'f' => $list ? $list->result_array() : [],
            'u' => $update,
        ];
        $this->print_json($data);
    }
    ...
    public function sendchat()
    {
        ...
        if (!isset($_SESSION['chat'][$this->user->chat_id]['history'][$to])) {
            $_SESSION['chat'][$this->user->chat_id]['history'][$to] = [];
        }
        $data = [
            's' => 1,
            'f' => $to,
