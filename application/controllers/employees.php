...
class employees extends Person_controller
{
    ...
    */
    ...
    public function suggest($minimum = false)
    {
        if ($minimum) {
    ...
    */
    ...
    public function save($employee_id = -1)
    {
        if ($employee_id == -1 && $this->Employee->exists('', $this->input->post('username'))) {
            echo json_encode(['success' => false, 'message' => $this->lang->line('employees_username_exist')]);
        ...
        foreach ($days as $day) {
            ...
            if ($status) {
                $person_schedule[] = ['day' => $day, 'in' => $this->input->post('in'.$day).':00:00', 'out' => $this->input->post('out'.$day).':00:00'];
            }
        };
        ...
        $permission_data = $this->input->post('permissions') != false ? $this->input->post('permissions') : [];
        $full_permission_data = [];
        foreach ($permission_data as $subpermission) {
            $subpermissions = $this->input->post($subpermission) != false ? $this->input->post($subpermission) : [];
            $full_permission_data[$subpermission] = (count($subpermissions) > 0) ? implode(',', $subpermissions) : 'none';
        }

        if ($employee_id == -1) {
            $nameuser_emer = $this->input->post('username');
        } else {
            $nameuser_emer = $this->input->post('nameuser');
        }
        ...
        if ($this->input->post('password') != '') {
            ...
            $employee_data = ['username' => $nameuser_emer, 'type_employees' => $this->input->post('employee_profile_type')];
        }
        if ($id = $this->Employee->save($person_data, $employee_data, $full_permission_data, $employee_id)) {
            if ($employee_id != -1) {
                ...
                if (file_exists('./images/employees/'.md5($id).'.jpg')) {
                    $a = true;
                } else {
                    $a = false;
                }
            } else {
                $msg_post_subt = $this->lang->line('employees_successful_updating');
                $a = false;
            }
            $tuId = md5($this->session->userdata('dblocation').'+'.$id);
            $dat = $this->uploadImagen_photo($tuId, $a);
            if ($this->Schedule->save($person_schedule, $id)) {
                echo json_encode(['success' => true, 'message' => $msg_post_subt.' '.$person_data['first_name'].' '.$person_data['last_name'], 'person_id' => $id, 'image' => $dat]);
            }
        } else {
            //failure

            echo json_encode(['success' => false, 'message' => $this->lang->line('employees_error_adding_updating').' '.
    ...
    */
    ...
    public function delete()
    {
        ...
        if ($this->Employee->delete_list($employees_to_delete)) {
            echo json_encode(['success' => true, 'message' => $this->lang->line('employees_successful_deleted').' '.
            count($employees_to_delete).' '.$this->lang->line('employees_one_or_multiple'), ]);
        } else {
            echo json_encode(['success' => false, 'message' => $this->lang->line('employees_cannot_be_deleted')]);
        }
    }
    ...
    */
    ...
    public function json_calendar($person_id = 0)
    {
        $year = date('Y');
        $month = date('m');
        $events_status = ['incomplete' => '#CA1C1C', 'notwork' => '#000', 'working' => '#0063C6'];
        $response = [];
    ...
    public function worked_report_inputs()
    {
        $months_worked = [];
    ...
    public function open_day()
    {
        ...
        //Respuesta
        $response = ['message' => 'Wrong Password or Username, try Again!', 'status' => 0];
    ...
    {
        $response = ['status' => 0, 'message' => 'General Error, Try Again!'];
    ...
    public function form_profile_employee($per = -1)
    {
        if ($per != -1) {
            $data['profile'] = $this->Employee->getProfile_Employee($per);
        } else {
            $data['profile'] = '';
        }
        $data['all_modules'] = $this->Module->get_all_modules();
    ...
    public function save_profile_employee($per = '')
    {
        if ($per == '' && $this->Employee->exists_profile($this->input->post('name'))) {
            echo json_encode(['success' => false, 'message' => $this->lang->line('employees_profile_per_name_e')]);

            return;
        }//pos-name
        if ($per == '') {
            $prodile_data = ['name' => $this->input->post('name'), 'new' => true];
        } else {
            if (!$this->input->post('name') && $per == 'administrator') {
                $name_profile = 'administrator';
            } else {
                $name_profile = $this->input->post('name');
            }
            $prodile_data = ['name' => $name_profile, 'new' => false, 'last' => $this->input->post('pos-name')];
        }
        ...
        if ($this->Employee->create_profile($prodile_data, $full_permission_data)) {
            //New employee
            if ($per == '') {
                echo json_encode(['success' => true, 'message' => $this->lang->line('employees_successful_adding').' '.
                $prodile_data['name'], 'person_id' => $prodile_data['name'], ]);
            } else {
                echo json_encode(['success' => true, 'message' => $this->lang->line('employees_successful_updating').' '.$prodile_data['name'], 'person_id' => $per]);
            }
        } else { //failure
            echo json_encode(['success' => false, 'message' => $this->lang->line('employees_error_adding_updating').' '.
            $person_data['first_name'].' '.$person_data['last_name'], 'person_id' => -1, ]);
