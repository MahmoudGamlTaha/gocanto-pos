...
class Sale_suspended extends CI_Model
{
        ...
        foreach ($items as $line => $item) {
            ...
            if ($customer_id == -1 or $customer->taxable) {
                foreach ($this->Item_taxes->get_info($item['item_id']) as $row) {
                    $this->con->insert('sales_suspended_items_taxes', [
                        'sale_id'     => $sale_id,
                        'item_id'     => $item['item_id'],
                        'line'        => $item['line'],
                        'name'        => $row['name'],
    ...
    public function delete($sale_id)
    {
        ...
        $this->con->delete('sales_suspended_payments', ['sale_id' => $sale_id]);
        $this->con->delete('sales_suspended_items_taxes', ['sale_id' => $sale_id]);
        $this->con->delete('sales_suspended_items', ['sale_id' => $sale_id]);
        $this->con->delete('sales_suspended', ['sale_id' => $sale_id]);
