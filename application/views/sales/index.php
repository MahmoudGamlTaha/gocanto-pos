...
<div id="filter-bar" id="titleTextImg" class="middle-gray-bar" style="display: none;">
	<div style="float:left;"><?php echo $this->lang->line('search_options') ?> :</div>
		...
		<?php	
        echo form_open('sales/index/'.$list['sale_type'], ['id' => 'orders_filter_form']);
        $labels = [
        ...
        foreach ($labels as $i => $value) {
            $barra .= ($barra == '' ? '' : '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;').form_radio(['name' => 'filters', 'id' => 'filters'.$i, 'value' => $i, 'checked' => ($list['cfilter'] == $i ? 1 : 0)]).'&nbsp;'.form_label($value, 'labelfilter'.$i);
        }
        echo $barra.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
