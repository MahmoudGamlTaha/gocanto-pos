...
<div id="table_holder">
    ...
    if ($i == 0) {
        $border_ra = 'border-top-left-radius:5px;-webkit-border-top-left-radius:5px;';
    } elseif (($i + 1) == $numTh) {
        $border_ra = 'border-top-right-radius:5px;-webkit-border-top-right-radius:5px;';
    } else {
        $border_ra = '';
    }
    ?>
				<th colspan="<?php echo $cs;
    ?>" style="padding: 5px;<?php echo $border_ra;
    ?>"><?php echo $header;
    ?></th>
				<?php $i++;
} ?>
    ...
    ?>
				<td colspan="<?php echo $cs;
    ?>" <?php echo $class;
    ?> style="<?php if ($export_excel) {
...
<!-- <div style="text-align:center;">
    ...
    ?>
		<div class="summary_row"><?php echo $this->lang->line('reports_'.$name).': '.to_currency($value);
    ?></div>
	<?php 
}?>
	</div>
