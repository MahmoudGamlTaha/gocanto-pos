...
class employee extends Person
{
    ...
    */
    ...
    public function exists($person_id, $name_user = '', $email = '')
    {
        $this->con->from('employees');
        $this->con->join('people', 'people.person_id = employees.person_id');
        if ($name_user != '') {
            $this->con->where('employees.username', $name_user);
        } else {
    ...
    */
    ...
    public function get_worked_details($person_id, $year = 0)
    {
    ...
    public function get_working_hours($person_id = 0, $date = false)
    {
    ...
    public function get_worked_days($person_id, $day = 0)
    {
    ...
    public function get_all_working()
    {
        $this->con->from('employees_schedule');
    ...
    */
    ...
    public function save(&$person_data, &$employee_data, &$permission_data, $employee_id = false)
    {
        $success = false;
        ...
        if ($idaux = parent::save($person_data, $employee_id)) {
            if (!$employee_id or !$this->exists($employee_id)) {
                ...
                if ($this->con->insert('employees', $employee_data)) {
                    $success = $employee_data['person_id'];
                }
            } else {
            ...
            if ($success) {
                ...
                if ($this->con->delete('permissions', ['person_id' => $employee_id])) {
    ...
    */
    ...
    public function delete($employee_id)
    {
        ...
        if ($this->con->delete('permissions', ['person_id' => $employee_id])) {
            $this->con->where('person_id', $employee_id);
            $success = $this->con->update('employees', ['deleted' => 1]);
        }
        $this->con->trans_complete();
    ...
    */
    ...
    public function get_search_suggestions_minimun($search, $limit = 5)
    {
        $suggestions = [];
    ...
    */
    ...
    public function can_work($person_id = 0)
    {
        $b = 1;
    ...
    */
    ...
    public function login($username, $password)
    {
        ...
        $this->con->where(['employees.username' => $username, 'employees.password' => md5($password)]);
    ...
    public function login_($username, $password)
    {
        ...
        $this->con->where(['employees.username' => $username, 'employees.password' => md5($password)]);
        $this->con->where('employees.deleted', 0);
        $this->con->where(['schedules.day' => date('l')]); //Verifica que trabaje ese dia
        $this->con->where('CURTIME() BETWEEN ospos_schedules.in AND ospos_schedules.out'); //Verifica que este en su horario
        $employee = $this->con->get();
    ...
    public function can_logout($person_id, $password)
    {
        $b = false;
        $this->con->from('employees');
        $this->con->where(['employees.person_id' => $person_id, 'employees.password' => md5($password)]);
        $employee = $this->con->get();
        if ($employee->num_rows() == 1) {
            $b = true;
        }
     ...
     */
    public function open_day($employee_id)
    {
        $b = 0;
        $data = [
        ...
        $ewn = ($this->session->userdata('employees_working_now')) ? $this->session->userdata('employees_working_now') :  [0];
     ...
     * @return [boolean] [True o False segun sea]
     */
    public function close_day($employee_id)
    {
        $b = false;
        ...
        if ($this->db->update('employees_schedule', ['logout' => date('H:i:s')])) {
            $b = true;
    ...
    */
    ...
    public function logout($otherUser = '')
    {
        $pageRedirect = 'login/index';
        if ($otherUser != '') {
            $pageRedirect .= '/'.$otherUser;
        }
    ...
    */
    ...
    public function has_permission($module_id, $person_id, $t = false)
    {
        ...
        if (!$t) {
            $query = $this->con->get_where('permissions', ['person_id' => $person_id, 'module_id' => $module_id], 1);
        } else {
            $query = $this->con->get_where('employees_profile', ['profile_name' => $person_id, 'module_id' => $module_id], 1);
        }

        return $query->num_rows() == 1;
    }

    public function has_privilege_permi($module_id, $person_id, $privilege_name, $t = false)
    {
        //if no module_id is null, allow access
        if ($module_id == null) {
            return true;
        ...
        if ($t) {
            ...
            $this->con->where(['profile_name' => $person_id, 'module_id' => $module_id]);
        } else {
            $this->con->from('permissions');
            $this->con->like('privileges', $privilege_name);
            $this->con->where(['person_id' => $person_id, 'module_id' => $module_id]);
        }
    ...
    public function has_privilege($privilege, $module_id)
    {
        $person_id = $this->get_logged_in_employee_info()->person_id;
        $permissions = $this->con->get_where('permissions', ['person_id' => $person_id, 'module_id' => $module_id], 1);
    ...
    public function getProfile_Employee($id = '', $t = false)
    {
        ...
        if ($id != '') {
            $this->con->where('profile_name', $id);
        }
        ...
        if ($num > 0) {
                ...
                foreach ($query->result() as $row) {
                    ...
                    if ($t) {
                        if ($arr[$i]['name'] == ucwords($row->profile_name)) {
                            ...
                            } elseif ($row->privileges == 'save') {
                                $arr[$i]['module'] .= ',#'.$row->module_id.'-'.$row->privileges;
                            }
                        } else {
                            $i++;
                            $arr[$i]['name'] = ucwords($row->profile_name);
                        ...
                        if ($arr[$i]['name'] == ucwords($row->profile_name)) {
                            ...
                            if ($row->privileges != 'none') {
                                $arr[$i]['module'] .= ' (<small>'.($row->privileges).'</small>)';
                            }
                        } else {
                            $i++;
                            $arr[$i]['name'] = ucwords($row->profile_name);
            ...
            return 'No profile avarible';
        }
    }

    public function exists_profile($person_id)
    {
        $this->con->from('employees_profile');
    ...
    public function create_profile($data, $permiso)
    {
        $success = false;
        //Now insert the new permissions
        if ($data['new'] || (!$data['new'] && $this->con->delete('employees_profile', ['profile_name' => $data['last']]))) {
