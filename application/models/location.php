...
class location extends CI_Model
{
 ...
 //        	$this->con = $this->db;
    ...
    public function get_location($location_id = false)
    {
        ...
        return ($query->num_rows() == 1) ? $query->row_array() : ['id' => '', 'name' => '', 'hostname' => 'localhost', 'username' => 'root', 'database' => '', 'dbdriver' => '', 'dbprefix' => '', 'active' => false];
    }
    ...
    public function exists($location_name, $location_database)
    {
        $search = ['database' => $location_database, 'name' => $location_name];
                    ...
                    if ($this->con->insert('locations', $location_data)) {
                        ...
                        if ($query) {
                            if (mysql_select_db($location_data['database'], $conn)) {
                                ...
                                $backup1 = &$this->dbutil->backup(['format' => 'sql', 'tables' => $tables, 'add_drop' => false]);
                                $backup2 = &$this->dbutil->backup(['format' => 'sql', 'add_insert' => false, 'add_drop' => false, 'ignore' => $tables]);
