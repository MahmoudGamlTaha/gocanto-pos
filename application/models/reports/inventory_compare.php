...
class Inventory_compare extends CI_Model
{
    ...
    {
        return [$lang['reports_item_id'] = 'Id', $this->lang->line('reports_item'), $lang['reports_item_in_stock'] = 'In Stock', $lang['reports_comment_item'] = 'Comment', $lang['reports_checked'] = 'Checked'];
    }
    ...
    {
        $b = false;
        if ($this->con->insert('items_report', $compare_data)) {
            $b = true;
        }
    ...
    public function save_inventory($obs)
    {
        ...
        if (!$this->exist_inventory()) {
            ...
            $data = ['date_register' => date('Y-m-d H:i:s'), 'observation' => $obs, 'person_id' => $employee_id];
            if ($this->con->insert('observation_inventories', $data)) {
                $b = true;
            }
        }
