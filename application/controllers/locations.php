...
class locations extends Secure_area
{
    ...
    public function view($location_id = 0)
    {
        ...
        $data['dbdrivers'] = ['mysql' => 'mysql', 'mysqli' => 'mysqli', 'postgre' => 'postgre', 'odbc' => 'odbc', 'sqlite' => 'sqlite', 'oci8' => 'oci8'];
    ...
    public function save($location_id = 0)
    {
        ...
        if (is_bool($response) === true) {
            if ($response) { //Locacion previa
                echo json_encode(['success' => true, 'message' => $this->lang->line('location_updated'), 'location_id' => $location_id]);
            } else {
                echo json_encode(['success' => false, 'message' => $this->lang->line('location_no_updated'), 'location_id' => $location_id]);
            }
        } else {//New location
            if ($response > 0) {
                echo json_encode(['success' => true, 'message' => $this->lang->line('location_created'), 'location_id' => $location_id]);
            } elseif ($response == 0) {
                echo json_encode(['success' => false, 'message' => $this->lang->line('location_connection_error'), 'location_id' => $response]);
            } elseif ($response == -1) {
                echo json_encode(['success' => false, 'message' => $this->lang->line('location_create_error'), 'location_id' => $response]);
            } elseif ($response == -2) {
                echo json_encode(['success' => false, 'message' => $this->lang->line('location_create_db_error'), 'location_id' => $response]);
            }
        }
    }
