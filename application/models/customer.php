...
class customer extends Person
{
    ...
    */
    ...
    { //echo '---('.$search.')---';
        ...
        if (is_array($search)) {
            ...
            $this->con->where('customers.person_id', $search);
        } //person_id
        $query = $this->con->get();
    ...
    */
    ...
    public function get_all_credits($limit = 10000, $offset = 0, $customer_id = 0)
    {
    ...
    public function get_type_credits($customer_id)
    {
        $this->con->select('credit_id, person_custo_id, payment_amount, payment_period, day_pay, type, status');
        $this->con->from('credits');
        $this->con->where('person_custo_id', $customer_id);
    ...
    public function get_advance_credits($customer_id)
    {
    ...
    public function change_advance_credit($credit_id)
    {
        $this->con->where('person_custo_id', $credit_id);
        $this->con->where('status', 'advance');
        $success = $this->con->update('credits', ['status' => 'advance ready']);
    }

    public function change_status_credit($credit_id)
    {
        $this->con->where('credit_id', $credit_id);
        $success = $this->con->update('credits', ['status' => 'paid']);
    }
    ...
    public function count_all_credits($customer_id)
    {
        $this->con->from('credits');
        $this->con->where('person_custo_id', $customer_id);
    ...
    public function sum_all_credits($customer_id)
    {
        $this->con->select('SUM(payment_amount) as sum');
        $this->con->from('credits');
        $this->con->where('person_custo_id', $customer_id);
    ...
    */
    ...
    public function get_info_credits($customer_id)
    {
        $this->con->from('credits');
        $this->con->join('people', 'people.person_id = credits.person_custo_id');
    ...
    */
    ...
    public function get_info_by_name($customer_name)
    {
        $this->con->from('customers');
        $this->con->join('people', 'people.person_id = customers.person_id');
    ...
    */
    ...
    public function save(&$person_data, &$customer_data, $customer_id = false)
    {
        $success = false;
    ...
    public function save_credit(&$credit_data, $date_end = -1)
    {
        $success = false;
    ...
    public function check_final_date_credit($customer_id)
    {
    ...
    */
    ...
    public function delete($customer_id)
    {
        ...
        return $this->con->update('customers', ['deleted' => 1]);
    }
