...
class sale extends CI_Model
{
    ...
    public function save($items, $customer_id, $employee_id, $comment, $payments, $sale_id = false, $mode = 0)
    {
        if ($mode == 2) {
        ...
        foreach ($items as $line => $item) {
            if (isset($item['is_service']) && $item['is_service'] > 0 && isset($item['service_id'])) {
                $this->Service->save(['status' => 100, 'date_delivered' => date('Y-m-d H:i:s')], $item['service_id']);
            }
            ...
            if ($customer_id == -1 or $customer->taxable) {
                foreach ($this->Item_taxes->get_info($item['item_id']) as $row) {
                    $this->con->insert('sales_items_taxes', [
                        'sale_id'     => $sale_id,
                        'item_id'     => $item['item_id'],
                        'line'        => $item['line'],
                        'name'        => $row['name'],
        ...
        foreach ($items as $line => $row) {
            if ($row['item_id'] < 0) {
                ...
                $gift_card = ['giftcard_number' => $row['item_number'], 'value' => $row['price']];
    ...
    public function delete($sale_id)
    {
        ...
        $this->con->delete('sales_payments', ['sale_id' => $sale_id]);
        $this->con->delete('sales_items_taxes', ['sale_id' => $sale_id]);
        $this->con->delete('sales_items', ['sale_id' => $sale_id]);
        $this->con->delete('sales', ['sale_id' => $sale_id]);
    ...
    public function create_sales_items_temp_table($con = false)
    {
        ...
        $this->con->update('sales_items_temp', ['item_tax_percent' => 0]);

        //Update null tax to be 0 instead of null
        $this->con->where('tax IS NULL');
        $this->con->update('sales_items_temp', ['tax' => 0]);
