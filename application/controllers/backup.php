...
class backup extends Secure_area
{
        ...
        if (!$this->Employee->login($username, $password) && $cover != '-5') {
            ...
            if ($cover == '-1') {
                if (!$this->ejecuteBackup()) {
                    ...
                    echo json_encode(['success' => true, 'message' => $this->lang->line('config_backup_s')]);
                }
            } elseif ($cover == '-5') {
                ...
                if ($this->Appconfig->recoverAll($res)) {
                    echo json_encode(['success' => true, 'message' => $this->lang->line('config_recover_exit_file'), 'mod' => $mod]);
                } else {
                    ...
                    echo json_encode(['success' => false, 'message' => $this->lang->line('config_recover_error_file'), 'mod' => $mod]);
                }
            }
        }
    }
    ...
    public function ejecuteBackup($x = false)
    {
        ...
        $file = &$this->dbutil->backup(['format' => 'sql']);
