...
class giftcard extends CI_Model
{
    ...
    */
    ...
    public function exists($giftcard_id, $giftcard_number = false)
    {
        $query = false;
        if ($giftcard_id) {
    ...
    */
    ...
    public function save(&$giftcard_data, $giftcard_id = false, $add = false)
    {
        $update = false;
        ...
        if ($query = $this->exists($giftcard_id, $giftcard_data['giftcard_number'])) {
            ...
            if (!$value) {
                $giftcard_data['value'] = 0;
            }
            //si es adición, se le suma el valor actual
            if ($add) {
                $giftcard_data['value'] += $row->value;
            }
            //si ha cambiado el valor lo actualizamos
            if ($giftcard_data['value'] != $row->value) {
                $update = true;
            }
        } else {
            $giftcard_id = false;
        ...
        if (!$giftcard_id) {//si no existe la intentamos crear
            if (!$value) {
                $giftcard_data['value'] = 0;
            } //si no hay valor la iniciamos en cero
    ...
    */
    ...
    public function delete($giftcard_id)
    {
        ...
        return $this->con->update('giftcards', ['deleted' => 1]);
    }
    ...
    */
    ...
    {
        $this->con->where('giftcard_number', $giftcard_number);
        $this->con->update('giftcards', ['value' => $value]);
    }
}
