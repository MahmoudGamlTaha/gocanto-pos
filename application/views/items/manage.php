...
<div id="title_bar">
	...
	<div id="new_button">
        ...
        ?>
		<?php echo anchor("$controller_name/excel_import/width:450/height:165", '<span>Excel Import</span>', ['class' => 'big_button thickbox', 'title' => 'Import Items from Excel'])?>
	</div>
</div>
...
if ($this->Employee->isAdmin()) {
    ...
    echo form_open("$controller_name/set_location", ['id' => 'form_items_location']);
	...
	<div style="float:left;"><?php echo $this->lang->line('search_options') ?> :</div>
		...
		<?php      echo form_open("$controller_name/index", ['id' => 'items_filter_form']);
                echo '<input type="hidden" name="segu" value="1">';
                 ...
                 echo form_checkbox(['name' => 'low_inventory', 'id' => 'low_inventory', 'value' => 1, 'checked' => isset($low_inventory) ?  (($low_inventory) ? 1 : 0) : 0]).' | ';
                 echo form_label($this->lang->line('items_serialized_items').' '.':', 'is_serialized');
                 echo form_checkbox(['name' => 'is_serialized', 'id' => 'is_serialized', 'value' => 1, 'checked' => isset($is_serialized) ?  (($is_serialized) ? 1 : 0) : 0]).' | ';
                 echo form_label($this->lang->line('items_no_description_items').' '.':', 'no_description');
                 echo form_checkbox(['name' => 'no_description', 'id' => 'no_description', 'value' => 1, 'checked' => isset($no_description) ?  (($no_description) ? 1 : 0) : 0]);
                 echo form_close();
        ?>
...
<div id="table_action_header">
	<ul>
		...
		<li class="float_left"><?=form_button(['href' => "index.php/$controller_name/delete", 'id' => 'delete', 'class' => 'small_button'], $this->lang->line('common_delete'))?></li>
		<?php endif ?>
		<li class="float_left"><?=anchor("$controller_name/bulk_edit/width:$form_width", $this->lang->line('items_bulk_edit'), ['id' => 'bulk_edit', 'title' => $this->lang->line('items_bulk_edit'), 'class' => 'small_button'])?></li>
		<li class="float_left"><?=anchor("$controller_name/generate_barcodes", $this->lang->line('items_generate_barcodes'), ['id' => 'generate_barcodes', 'class' => 'small_button', 'target' => '_blank', 'title' => $this->lang->line('items_generate_barcodes')])?></li>
