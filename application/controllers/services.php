...
class services extends Secure_area //class Services extends CI_Controller
{
    ...
    public function find_service_info()
    {
    ...
    */
    ...
    public function suggest2()
    {
        ...
        if ($services) {
            foreach ($services->result() as $row) {
                $result[] = ['term' => $this->input->get('term'), 'id' => $row->service_id, 'text' => $row->first_name.' '.$row->last_name.', '.$row->brand_name.' - '.$row->model_name];
            }
        }
    ...
    public function count_details($service_id = -1)
    {
        $data['service_info'] = $this->Service->get_info($service_id);
    ...
    public function save($service_id = -1)
    {
        $person_id = $this->Service->exists_person($this->input->post('name'));
        if (!$person_id && $this->input->post('name')) {
            echo json_encode(['success' => false, 'message' => $this->lang->line('services_error_adding_person'), 'service_id' => -1, 'noOw' => true]);
        } else {
            $add_pay = '';
            $payments = 0;
            ...
            if ($service_id != -1) {
                ...
                if ($this->input->post('first_name')) {
                    ...
                    $customer_data = ['account_number' => null, 'taxable' => 0];
                ...
                if (!$data) {
                    echo json_encode(['success' => false, 'message' => $this->lang->line('services_error_adding_person'), 'service_id' => -1, 'noAddOw' => true]);
                } elseif (is_array($data)) {
                    $person_id = isset($data['add']) ? $data['add'] : $data['update'];
                }
            ...
            if ($service) {
                ...
                if ($service_id == -1) {//New service
                    echo json_encode(['success' => true, 'message' => $this->lang->line('services_successful_adding'), 'service_id' => $service]);
                } else {//previous service
                    echo json_encode(['success' => true, 'message' => $this->lang->line('services_successful_updating'), 'service_id' => $service_id, 'test' => $service_data]);
                }
            } else {//failure
                echo json_encode(['success' => false, 'message' => $this->lang->line('services_error_adding_updating').' '.$service_data['person_id'], 'service_id' => -1]);
            }
        }
    }

    public function suggest_models($brand = '')
    {
        $suggestions = $this->Service->suggest_model($this->input->post('q'), $brand);
        echo implode("\n", $suggestions);
    }

    public function suggest_brand($module = '')
    {
        $suggestions = $this->Service->suggest_brand($this->input->post('q'));
        echo implode("\n", $suggestions);
    }

    public function suggest_owner($module = '')
    {
        $suggestions = $this->Service->suggest_owner($this->input->post('q'));
        echo implode("\n", $suggestions);
    }

    public function suggest_items()
    {
    ...
    public function suggest3()
    {
        ...
        if ($items) {
            foreach ($items->result() as $row) {
                if ($row->item_id) {
                    $brand = $this->Service->suggest_model_brand($row->item_id);
                }
                if (!isset($brand) || count($brand) == 0) {
                    $brand = [0 => '', 1 => ''];
                }
                $result[] = ['id' => $row->item_id, 'item_number' => $row->item_number, 'text' => $row->name, 'qty' => $row->quantity, 'reorder_level' => $row->reorder_level, 'brand_name' => $brand[1], 'model_name' => $brand[0], 'price' => $row->unit_price];
            }
        }
        die(json_encode($result));
    }

    public function notes_view($service_id)
    {
        $data['service_info'] = $this->Service->get_info($service_id);
        $data['item_list_json'] = $this->Service->get_items($service_id);
        $items = $data['item_list_json']['id'];
