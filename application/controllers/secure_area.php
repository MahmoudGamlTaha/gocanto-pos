...
class Secure_area extends CI_Controller
{
    ...
    */
    ...
    public function __construct($module_id = null)
    {
        ...
        if ($module_id != 'items') {
            $this->session->unset_userdata('items_fil');
        }
        if ($module_id != 'services') {
            $this->session->unset_userdata('services_fil');
        }
        if ($module_id != 'receivings') {
            $this->session->unset_userdata('receiving_fil');
        }
        switch ($module_id) {
            ...
            break;
            case 'sales':
                if ($this->uri->segment(3) && $this->uri->segment(3) === 'shipping') {
                    $action = 'Shipping';
                } else {
                    $action = '';
                }
            break;
            case 'employees':
                if ($this->uri->segment(2) && $this->uri->segment(2) === 'assistance') {
                    $action = 1;
                } else {
                    $action = '';
                }
        ...
        if ($action === '') {
            if (!$this->Employee->has_permission($module_id, $id_employee_l)) {
                redirect('no_access/'.$module_id);
            }
        } elseif (isset($report)) {
            if (!$this->Employee->has_privilege($report, 'notification_alert')) {
                redirect('no_access/'.$report.'/1');
            }
        } elseif ($action !== 1) {
            if (!$this->Employee->has_privilege($action, 'stock_control')) {
                redirect('no_access/'.$action.'/1');
            }
        }
        ...
        if ($module_id != 'invetories_compare') {
            ...
            if ($this->Employee->has_privilege('Low Stock', 'notification_alert')) {
                ...
                $data['notifications']['inventory_low']['data'] = $this->Inventory_low->getData([]);
            }
            ...
            if ($this->Employee->has_privilege('Accounts Payable', 'notification_alert')) {
                ...
                $data['notifications']['accounts_payable']['data'] = array_merge($this->Detailed_receivings->getData([], true), $this->Transfers->transfers_receivable());
            }
            ...
            if ($this->Employee->has_privilege('Pendig Orders', 'notification_alert')) {
                ...
                $data['notifications']['pending_orders']['data'] = $this->Order->get_all(['filter_location' => $this->session->userdata('dblocation')]);
            }
