...
class items extends Secure_area implements iData_controller
{
    ...
    public function suggest2()
    {
        ...
        if ($item_kits) {
            foreach ($item_kits->result() as $row) {
                $result[] = ['id' => $row->item_kit_id, 'text' => 'Item Kit: '.$row->name, 'item_kit' => true];
            }
        }

        if ($items) {
            foreach ($items->result() as $row) {
                $result[] = ['id' => $row->item_id, 'item_number' => $row->item_number, 'text' => $row->name, 'qty' => $row->quantity, 'reorder_level' => $row->reorder_level];
            }
        }
    ...
    */
    ...
    public function view($item_id = -1, $action = false)
    {
        $data['item_info'] = $this->Item->get_info($item_id);
        $data['item_tax_info'] = $this->Item_taxes->get_info($item_id);
        $suppliers = ['' => $this->lang->line('items_none')];
        if ($action) {
            switch ($action) {
 ...
 //------------------------------------------- Ramel
    ...
    public function generate_barcodes($item_ids)
    {
        ...
        foreach ($item_ids as $item_id) {
            ...
            $result[] = ['name' => $item_info->name, 'id' => $item_id];
        }
    ...
    public function bulk_edit()
    {
        ...
        $data['suppliers'] = $suppliers;
        $data['allow_alt_desciption_choices'] = [
    ...
    public function save($item_id = false, $action = false)
    {
        ...
        if ($id) {
            ...
            //New item
            if ($item_id === false) {
                echo json_encode(['success' => true, 'message' => $this->lang->line('items_successful_adding').' '.
                $item_data['name'], 'item_id' => $id, 'pictures' => $dat, 'action' => $action, ]);
                // $item_id = $item_data['item_id'];
            } else { //previous item
                echo json_encode(['success' => true, 'message' => $this->lang->line('items_successful_updating').' '.
            ...
            for ($k = 0; $k < count($tax_percents); $k++) {
                if (is_numeric($tax_percents[$k])) {
                    $items_taxes_data[] = ['name' => $tax_names[$k], 'percent' => $tax_percents[$k]];
                }
            }
            ...
            echo json_encode(['success' => false, 'message' => $this->lang->line('items_error_adding_updating').' '.
    ...
    public function save_inventory($item_id = -1, $db = null)
    {
        $employee_id = $this->Employee->get_logged_in_employee_info()->person_id;
        $cur_item_info = $this->Item->get_info($item_id);
        $inv_data = 
        ...
        if ($db != '...' && $db != false) {
            if ($this->input->post('newquantity') < $cur_item_info->quantity) { //Para que no pase mas de lo que tiene
                ...
                if ($this->Item->save_in_other_inventory($item_data, $item_id, $db)) {    //Si incremento en la otra tienda
                    ...
                    echo json_encode(['success' => true, 'message' => 'Item traspasest To'.' "'.$db.'" '.
                    $cur_item_info->name, 'item_id' => $item_id, ]);
                } else {                                                                //Si no icremente en la otra tienda
                    echo json_encode(['success' => false, 'message' => 'This item may not exist in the destination or location that no longer exists']);
                }
            } else {
                echo json_encode(['success' => false, 'message' => 'We can not transfer this amount']);
            }
        } else {
...
//---------------------------------------------------------------------Ramel
    ...
    public function bulk_update()
    {
        ...
        foreach ($_POST as $key => $value) {
            ...
            } elseif ($value != '' and !(in_array($key, ['item_ids', 'tax_names', 'tax_percents']))) {
                $item_data["$key"] = $value;
            }
        }
        ...
        if (empty($item_data) || $this->Item->update_multiple($item_data, $items_to_update)) {
            ...
            echo json_encode(['success' => true, 'message' => $this->lang->line('items_successful_bulk_edit')]);
        } else {
            echo json_encode(['success' => false, 'message' => $this->lang->line('items_error_updating_multiple')]);
        }
    }
    ...
    public function delete()
    {
        ...
        if ($this->Item->delete_list($items_to_delete)) {
            echo json_encode(['success' => true, 'message' => $this->lang->line('items_successful_deleted').' '.
            count($items_to_delete).' '.$this->lang->line('items_one_or_multiple'), ]);
        } else {
            echo json_encode(['success' => false, 'message' => $this->lang->line('items_cannot_be_deleted')]);
        }
    }
    ...
    public function do_excel_import()
    {
        ...
        if ($_FILES['file_path']['error'] != UPLOAD_ERR_OK) {
            ...
            if (($handle = fopen($_FILES['file_path']['tmp_name'], 'r')) !== false) {
                ...
                while (($data = fgetcsv($handle)) !== false) {
                    ...
                    if ($this->Item->save($item_data)) {
                        ...
                        if (is_numeric($data[7]) && $data[6] != '') {
                            $items_taxes_data[] = ['name' => $data[6], 'percent' => $data[7]];
                        }

                        //tax 2
                        if (is_numeric($data[9]) && $data[8] != '') {
                            $items_taxes_data[] = ['name' => $data[8], 'percent' => $data[9]];
                        }
                ...
                echo json_encode(['success' => false, 'message' => 'Your upload file has no data or not in supported format.']);
    ...
    public function report_item_broken($item_id)
    {
        if ($this->Item->report_broken($item_id)) {
            echo json_encode(['success' => true, 'message' => 'Item reported']);
        } else {
    ...
    public function uploadImagen_photo($id, $e)
    {
        ...
        try {
            ...
            if (count($nameLogo) > 0) {
                ...
                $data = ['upload_status' => 1, 'upload_message' => $nameLogo];
            } else {
    ...
    public function delete_picture($id, $i)
    {
