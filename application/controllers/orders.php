...
class orders extends Secure_area
{
    ...
    public function index($low_stock = false)
    {
        //Tab 1
        if ($low_stock) {
        ...
        $data['location_lists'] = array_merge(['' => 'Location'], $data['location_lists']);
        ...
        $data['status'] = [$this->lang->line('orders_status1'), $this->lang->line('orders_status2')];
    ...
    {
        $response = ['status' => true, 'messagge' => $this->lang->line('orders_delete_item')];
        $this->order_lib->delete_item($item_number);
        die(json_encode($response));
    }

    public function save($sale_id = false)
    {
        $response = ['status' => false, 'message' => $this->lang->line('orders_no_save')];
    ...
    {
        $response = ['status' => false, 'message' => 'Nose a borrado'];
    ...
    public function check_availability($id_order)
    {
        $this->load->model('Order');
        $items = $this->Order->check_availability($id_order);
        if ($items === 1) {
            redirect('sales/pending_orders_to_shipping/'.$id_order);
        } else {
            redirect('reports/pending_orders/'.$id_order.'/1:'.$items);
        }
    }
}
