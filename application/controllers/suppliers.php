<?php

require_once 'person_controller.php';
class suppliers extends Person_controller
{
    public function __construct()
    {
        parent::__construct('suppliers');
    }

    public function index()
    {
        $config['base_url'] = site_url('/suppliers/index');
        $config['total_rows'] = $this->Supplier->count_all();
        $config['per_page'] = '20';
        $config['uri_segment'] = 3;
        $this->pagination->initialize($config);

        $data['controller_name'] = strtolower(get_class());
        $data['form_width'] = $this->get_form_width();
        $data['manage_table'] = get_supplier_manage_table($this->Supplier->get_all($config['per_page'], $this->uri->segment($config['uri_segment'])), $this);
        $this->load->view('suppliers/manage', $data);
    }

    /*
    Returns supplier table data rows. This will be called with AJAX.
    */

    public function search()
    {
        $search = $this->input->post('search');
        $data_rows = get_supplier_manage_table_data_rows($this->Supplier->search($search), $this);
        echo $data_rows;
    }

    /*
    Gives search suggestions based on what is being searched for
    */

    public function suggest()
    {
        $suggestions = $this->Supplier->get_search_suggestions($this->input->post('q'), $this->input->post('limit'));
        echo implode("\n", $suggestions);
    }

    /*
    Loads the supplier edit form
    */

    public function view($supplier_id = -1)
    {
        $data['person_info'] = $this->Supplier->get_info($supplier_id);
        $this->load->view('suppliers/form', $data);
    }

    public function exists_email($email = '')
    {
        $email = $this->input->post('email');
        if ($this->Supplier->exists_email($email) != '0') {
            //si existe
            echo json_encode(['success' => true, 'message' => 'existe', 'email' => $this->Supplier->exists_email($email)]);
        } else {
            //no existe
            echo json_encode(['success' => false, 'message' => 'no existe', 'email' => $this->Supplier->exists_email($email)]);
        }
    }

    /*
    Inserts/updates a supplier
    */

    public function save($supplier_id = -1)
    {
        if ($supplier_id == -1) {
            if ($this->Supplier->exists_email($this->input->post('email')) != '0') {
                //si existe
                die(
                    json_encode(
                        [
                            'success' => true,
                            'message' => 'existe',
                            'email'   => $this->Supplier->exists_email($this->input->post('email')),
                        ]
                    )
                );
            } else {
                $person_data['email'] = $this->input->post('email');
            }
        } else {
            if ($this->Supplier->exists_email($this->input->post('email')) == '0') {
                $person_data['email'] = $this->input->post('email');
            }
        }

        $person_data['first_name'] = $this->input->post('first_name');
        $person_data['last_name'] = $this->input->post('last_name');
        $person_data['phone_number'] = $this->input->post('phone_number');
        $person_data['address_1'] = $this->input->post('address_1');
        $person_data['address_2'] = $this->input->post('address_2');
        $person_data['city'] = $this->input->post('city');
        $person_data['state'] = $this->input->post('state');
        $person_data['zip'] = $this->input->post('zip');
        $person_data['country'] = $this->input->post('country');
        $person_data['comments'] = $this->input->post('comments');

        $supplier_data = [
            'company_name'     => $this->input->post('company_name'),
            'account_number'   => null,
            'bank_info'        => $this->input->post('bank_info'),
            'work_phone'       => $this->input->post('work_phone'),
            'product_supplied' => $this->input->post('prod_supplied'),
            'discounts'        => $this->input->post('discount'),
        ];

        if ($this->Supplier->save($person_data, $supplier_data, $supplier_id)) {
            //New supplier
            if ($supplier_id == -1) {
                echo json_encode(['success' => true, 'message' => $this->lang->line('suppliers_successful_adding').' '.
                $supplier_data['company_name'], 'person_id' => $supplier_data['person_id'], ]);
            } else {
                //previous supplier

                echo json_encode(['success' => true, 'message' => $this->lang->line('suppliers_successful_updating').' '.
                $supplier_data['company_name'], 'person_id' => $supplier_id, ]);
            }
        } else {
            //failure

            echo json_encode(['success' => false, 'message' => $this->lang->line('suppliers_error_adding_updating').' '.
            $supplier_data['company_name'], 'person_id' => -1, ]);
        }
    }

    /*
    This deletes suppliers from the suppliers table
    */

    public function delete()
    {
        $suppliers_to_delete = $this->input->post('ids');

        if ($this->Supplier->delete_list($suppliers_to_delete)) {
            echo json_encode(['success' => true, 'message' => $this->lang->line('suppliers_successful_deleted').' '.
            count($suppliers_to_delete).' '.$this->lang->line('suppliers_one_or_multiple'), ]);
        } else {
            echo json_encode(['success' => false, 'message' => $this->lang->line('suppliers_cannot_be_deleted')]);
        }
    }

    /*
    Gets one row for a supplier manage table. This is called using AJAX to update one row.
    */

    public function get_row()
    {
        $person_id = $this->input->post('row_id');
        $data_row = get_supplier_data_row($this->Supplier->get_info($person_id), $this);
        echo $data_row;
    }

    /*
    get the width for the add/edit form
    */

    public function get_form_width()
    {
        return '570/height:425';
    }
}
