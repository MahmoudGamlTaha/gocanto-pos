...
<?php 
	...
	<div id='report_sale_type' style="padding: 0 0 0 25px">
		<?php echo form_dropdown('sale_type', ['all' => $this->lang->line('reports_all'), 'sales' => $this->lang->line('reports_sales'), 'returns' => $this->lang->line('reports_returns')], 'all', 'id="sale_type"'); ?>
	</div>
...
$(document).ready(function()
{
	$("#generate_report").click(function()
	{
		...
		{
			window.location = window.location+'/'+$("#report_date_range_simple option:selected").val() + '/'+sale_type+'/'+export_excel<?php if ($this->Employee->isAdmin()) {
    ?>+'/'+$('#locationbd').val()<?php 
} ?>;
		}
		else
		{
			...
			window.location = window.location+'/'+start_date + '/'+ end_date + '/'+sale_type+'/'+ export_excel<?php if ($this->Employee->isAdmin()) {
    ?>+'/'+$('#locationbd').val()<?php 
} ?>;
		}
	});
