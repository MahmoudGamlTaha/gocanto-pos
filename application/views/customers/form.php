<?php 
    echo form_open_multipart('customers/save/'.$person_info->person_id, ['id' => 'customer_form']);
...
<div>
	...
	<div class="field_row clearfix">
		<div style="width: 100%; float: left">
			<div class="field_row clearfix">
				<?php 
                    ...
                    echo form_label($this->lang->line('customers_taxable').':', 'taxable', ['class' => 'lable-form', 'style' => 'float:none']);
                ?>
			</div>
		</div>
		<div style="width: 180px; float: left">	
			<?php echo form_label($this->lang->line('customers_lbl_porce_discount').':', 'discount', ['class' => 'lable-form']); ?>
			<div>
			<?php echo form_input([
                'name'  => 'discount',
                'id'    => 'discount',
		...
		<div style="width: 180px; float: left">	
			<?php echo form_label($this->lang->line('customers_lbl_customer_type').':', 'customer_type', ['class' => 'lable-form']); ?>
			<div>
				<select id="customer_type" name="customer_type">
					<option value="0" <?php if ($person_info->type == '0') {
    echo 'selected';
} ?> ><?=$this->lang->line('customers_cbo_type_regular')?></option>
					<option value="1" <?php if ($person_info->type == '1') {
    echo 'selected';
} ?> ><?=$this->lang->line('customers_cbo_type_company')?></option>
				</select>
			</div>
		</div>
		<div style="width: 180px; float: left">	
			<?php echo form_label($this->lang->line('customers_lbl_tax_id').':', 'tax_id', ['class' => 'lable-form']); ?>
			<div>
			<?php echo form_input([
                'name'  => 'tax_id',
                'id'    => 'tax_id',
    ...
    echo form_label('Give Credit'.':', 'give_credit', ['class' => 'lable-form', 'style' => 'float:none']);
    ?>
			</div>
		</div>
		...
		<div  id="credit_panel" style="display:none">
			...
			<div class="field_row clearfix">
				<div style="width: 180px; float: left">	
					<?php echo form_label('Amount:', 'amount', ['class' => 'lable-form']);
    ?>
					<div>
					<?php echo form_input([
                        'name'  => 'amount',
    ...
    ?>
				...
				<div style="width: 180px; float: left">	
					<?php echo form_label('Payment Term:', 'payment_term', ['style' => 'width:130px', 'class' => 'lable-form']);
    ?>
					<div>
						<select id="amount_dyas" name="amount_dyas">
							<option value="0" <?php if ($amount[1] == '') {
    echo 'selected';
}
    ?>>...</option>
							<option value="30_days" <?php if ($amount[1] == '30_days') {
    echo 'selected';
}
    ?>><?=$this->lang->line('30_days')?></option>
							<option value="60_days" <?php if ($amount[1] == '60_days') {
    echo 'selected';
}
    ?>><?=$this->lang->line('60_days')?></option>
							<option value="90_days" <?php if ($amount[1] == '90_days') {
    echo 'selected';
}
    ?>><?=$this->lang->line('90_days')?></option>
						</select>
					</div>
				</div>
