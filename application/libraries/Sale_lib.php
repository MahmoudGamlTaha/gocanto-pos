...
class Sale_lib
{
    ...
    public function get_payments($format = false)
    {
        ...
        if ($format && count($this->CI->session->userdata($this->nameS.'payments')) > 0) {
            foreach ($this->CI->session->userdata($this->nameS.'payments') as $key) {
                $payments[$key['payment_type']] = ['payment_type'      => str_replace('_', ' ', $key['payment_type']),
                                                      'payment_amount' => $key['payment_amount'], ];
            }
        } else {
            $payments = $this->CI->session->userdata($this->nameS.'payments');
        }
    ...
    public function add_item($item_id, $quantity = 1, $discount = 0, $price = null, $description = null, $serialnumber = null, $service_id = null)
    {
        ...
        if ($this->get_mode() == 'shipping') {
            $price = $item_info->cost_price;
        } //si es shipping se cobra el item a su precio de costo
        $array = [
            'item_id'                  => $item_id,
            'line'                     => $insertkey,
    ...
    public function out_of_stock($item_id)
    {
        //make sure item exists
        if (!$this->CI->Item->exists($item_id)) {
            //try to get item id given an item_number
    ...
    public function _edit_item($line, $campo, $valor)
    {
        $items = $this->get_cart();
        if (isset($items[$line])) {
    ...
    public function add_item_kit($external_item_kit_id)
    {
        //KIT #
    ...
    public function set_taxing($val)
    {
