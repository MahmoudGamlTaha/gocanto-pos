...
class giftcards extends Secure_area implements iData_controller
{
    ...
    */
    ...
    public function save($giftcard_id = false)
    {
        $selling = (bool) $this->input->post('giftcard_sale'); //detectar si es una venta
        $value = $this->input->post('value');
        $giftcard_number = $this->input->post('giftcard_number');
        $giftcard_data = ['giftcard_number' => $giftcard_number];
        //se le agrega el valor si no es una venta
        if (!$selling) {
            $giftcard_data['value'] = $value;
        }
    ...
    public function delete()
    {
        ...
        if ($this->Giftcard->delete_list($giftcards_to_delete)) {
            echo json_encode(['success' => true, 'message' => $this->lang->line('giftcards_successful_deleted').' '.
            count($giftcards_to_delete).' '.$this->lang->line('giftcards_one_or_multiple'), ]);
        } else {
            echo json_encode(['success' => false, 'message' => $this->lang->line('giftcards_cannot_be_deleted')]);
        }
    }
