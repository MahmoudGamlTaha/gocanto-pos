...
class receivings extends Secure_area
{
    ...
    public function complete($other = 0)
    {
        $this->load->model('Transfers');
        if ($this->session->userdata('from_rece_t')) {
            $other = $this->session->userdata('from_rece_t');
        }
    ...
    public function _reload($data = [])
    {
        ...
        $data_receive['modes'] = ['receive' => $this->lang->line('recvs_receiving'), 'return' => $this->lang->line('recvs_return')];
        ...
        if ($this->input->post() !== false) {
            ...
            switch ($filters) {
                case 1: $filters = ['sender' => $data['location']]; break;
                case 2: $filters = ['receiver' => $data['location']]; break;
            }
        ...
        $this->load->view('receivings/index', ['data_receive' => $data_receive, 'data' => $data]);
    }
