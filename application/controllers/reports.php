...
class reports extends Secure_area
{
    ...
    public function customers_credit_overdue_bills()
    {
        $data = $this->_get_common_report_data();
        $data['report_name'] = ucwords(str_replace('_', ' ', $this->uri->segment(2)));
        $data['customers_name'] = ucwords(str_replace('%20', ' ', $this->uri->segment(3)));
    ...
    public function credit_customers_data()
    {
        ...
        foreach ($locations as $location) {
            ...
            $report_data = $model->getData(['start_date' => $sd, 'end_date' => $ed, 'customer_id' => $this->input->post('customer_id')]);
            ...
            foreach ($report_data as $row) {
                ...
                $summary_data[] = [$row['day_pay'], $row['transfer_id'], $row['payment_amount'], $row['balance'], $p, ucwords($row['type']), ucwords($row['status'])];
            }
            ...
            $data = [
                ...
                'summary_data'  => $model->getSummaryData(['start_date' => $sd, 'end_date' => $ed, 'customer_id' => $this->input->post('customer_id')]),
                'export_excel'  => 0,
            ];
            $_data['list'][] = $data;
        }
    ...
    public function customers_credit_overdue_bills_data()
    {
        $loca = $this->input->post('locationbd');
        if ($loca) {
            $location = $loca;
        ...
        foreach ($locations as $location) {
            ...
            foreach ($report_data as $row) {
                $summary_data[] = [$row['credit_id'], $row['payment_amount'], str_replace('_', ' ', $row['payment_period']).' '.$this->lang->line('customers_period_days'), $row['day_pay'], ucwords($row['status']), ucwords($row['ended'])];
            }
            $customer_data = $model->get_info($this->input->post('customer_id'));
            $data = [
    ...
    public function date_input_excel_export()
    {
        $data = $this->_get_common_report_data();
        $data['report_name'] = ucwords(str_replace('_', ' ', $this->uri->segment(2)));
    ...
    public function get_locations($location = 'default')
    {
    ...
    public function summary_sales($start_date, $end_date, $sale_type, $export_excel = 0, $location = 'default')
    {
        ...
        foreach ($locations as $location) {
            ...
            $tabular_data = [];
            $report_data = $model->getData(['start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type]);
            foreach ($report_data as $row) {
                $tabular_data[] = [$row['sale_date'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']), to_currency($row['profit'])];
            }
            $data = [
                ...
                'data'         => $tabular_data,
                'location'     => ucwords(($location == 'default' ? 'Principal' : $location)),
                'summary_data' => $model->getSummaryData(['start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type]),
    ...
    public function summary_categories($start_date, $end_date, $sale_type, $export_excel = 0, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
        ...
        foreach ($locations as $location) {
            ...
            $report_data = $model->getData(['start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type]);
            foreach ($report_data as $row) {
                $tabular_data[] = [$row['category'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']), to_currency($row['profit'])];
            }
            $data = [
    ...
    public function summary_customers($start_date, $end_date, $sale_type, $export_excel = 0, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $locations = $loca;
        ...
        foreach ($locations as $location) {
            ...
            $report_data = $model->getData(['start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type]);
            foreach ($report_data as $row) {
                $tabular_data[] = [$row['customer'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']), to_currency($row['profit'])];
            }
            $data = [
    ...
    public function summary_suppliers($start_date, $end_date, $sale_type, $export_excel = 0, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
        ...
        foreach ($locations as $location) {
            ...
            $report_data = $model->getData(['start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type]);
            foreach ($report_data as $row) {
                $tabular_data[] = [$row['supplier'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']), to_currency($row['profit'])];
            }
            $data = [
    ...
    public function summary_items($start_date, $end_date, $sale_type, $export_excel = 0, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
        ...
        foreach ($locations as $location) {
            ...
            $report_data = $model->getData(['start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type]);
            foreach ($report_data as $row) {
                $tabular_data[] = [character_limiter($row['name'], 16), $row['quantity_purchased'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']), to_currency($row['profit'])];
            }
            $data = [
    ...
    public function summary_employees($start_date, $end_date, $sale_type, $export_excel = 0, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
        ...
        foreach ($locations as $location) {
            ...
            $report_data = $model->getData(['start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type]);
            foreach ($report_data as $row) {
                $tabular_data[] = [$row['employee'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']), to_currency($row['profit'])];
            }
            $data = [
    ...
    public function summary_taxes($start_date, $end_date, $sale_type, $export_excel = 0, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
        ...
        foreach ($locations as $location) {
            ...
            $report_data = $model->getData(['start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type]);
            foreach ($report_data as $row) {
                $tabular_data[] = [$row['percent'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax'])];
            }
            $data = [
    ...
    public function summary_discounts($start_date, $end_date, $sale_type, $export_excel = 0, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
        ...
        foreach ($locations as $location) {
            ...
            $report_data = $model->getData(['start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type]);
            foreach ($report_data as $row) {
                $tabular_data[] = [$row['discount_percent'], $row['count']];
            }
            $data = [
    ...
    public function summary_payments($start_date, $end_date, $sale_type, $export_excel = 0, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
    ...
    public function graphical_summary_sales($start_date, $end_date, $sale_type, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
    ...
    public function graphical_summary_sales_graph($start_date, $end_date, $sale_type, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
    ...
    public function graphical_summary_items($start_date, $end_date, $sale_type, $location = 'default')
    {
        $this->load->model('reports/Summary_items');
        $model = $this->Summary_items;
    ...
    public function graphical_summary_items_graph($start_date, $end_date, $sale_type, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
    ...
    public function graphical_summary_categories($start_date, $end_date, $sale_type, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
    ...
    public function graphical_summary_categories_graph($start_date, $end_date, $sale_type, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
    ...
    public function graphical_summary_suppliers($start_date, $end_date, $sale_type, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
    ...
    public function graphical_summary_suppliers_graph($start_date, $end_date, $sale_type, $location = 'default')
    {
    ...
    public function graphical_summary_employees($start_date, $end_date, $sale_type, $location = 'default')
    {
    ...
    public function graphical_summary_employees_graph($start_date, $end_date, $sale_type, $location = 'default')
    {
        $location = ($this->input->get('loc')) ? urldecode($this->input->get('loc')) : 'default';
        // if ($loca) $location=$loca;
        $this->load->model('reports/Summary_employees');
        ...
        $this->Receiving->con = $this->Sale->con;
        $this->Receiving->create_receivings_items_temp_table();
        $report_data = $model->getData(['start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => 'sales']);
        $graph_data = [];
        foreach ($report_data as $row) {
    ...
    public function graphical_summary_taxes($start_date, $end_date, $sale_type, $location = 'default')
    {
        $location = urldecode($this->input->get('loc'));
        if (isset($loca)) {
            $location = $loca;
    ...
    public function graphical_summary_taxes_graph($start_date, $end_date, $sale_type, $location = 'default')
    {
        $location = ($this->input->get('loc')) ? urldecode($this->input->get('loc')) : 'default';
        $this->load->model('reports/Summary_taxes');
        $model = $this->Summary_taxes;
    ...
    public function graphical_summary_customers($start_date, $end_date, $sale_type, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
    ...
    public function graphical_summary_customers_graph($start_date, $end_date, $sale_type, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
    ...
    public function graphical_summary_discounts($start_date, $end_date, $sale_type, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
    ...
    public function graphical_summary_discounts_graph($start_date, $end_date, $sale_type, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
    ...
    public function graphical_summary_payments($start_date, $end_date, $sale_type, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
    ...
    public function graphical_summary_payments_graph($start_date, $end_date, $sale_type, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
    ...
    public function specific_customer($start_date, $end_date, $customer_id, $sale_type = 'all', $export_excel = 0)
    {
        ...
        $this->Receiving->create_receivings_items_temp_table();
        $headers = $model->getDataColumns();
        $report_data = $model->getData(['start_date' => $start_date, 'end_date' => $end_date, 'customer_id' => $customer_id, 'sale_type' => $sale_type]);
        $summary_data = [];
        $details_data = [];
        foreach ($report_data['summary'] as $key => $row) {
            $summary_data[] = [anchor('sales/receipt/'.$row['sale_id'], 'POS '.$row['sale_id'], ['target' => '_blank']), $row['sale_date'], floor($row['items_purchased']), $row['employee_name'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']), to_currency($row['profit']), $row['payment_type'], $row['comment']];
            foreach ($report_data['details'][$key] as $drow) {
                $details_data[$key][] = [$drow['name'], $drow['category'], $drow['serialnumber'], $drow['description'], floor($drow['quantity_purchased']), to_currency($drow['subtotal']), to_currency($drow['total']), to_currency($drow['tax']), to_currency($drow['profit']), $drow['discount_percent'].'%'];
            }
        }
        ...
        $data = [
            ...
            'overall_summary_data' => $model->getSummaryData(['start_date' => $start_date, 'end_date' => $end_date, 'customer_id' => $customer_id, 'sale_type' => $sale_type]),
            'export_excel'         => $export_excel,
            'right'                => [2, 4, 5, 6, 7],
            'right_d'              => [4, 5, 6, 7, 8, 9],
        ];
    ...
    public function specific_employee($start_date, $end_date, $employee_id, $sale_type, $export_excel = 0)
    {
        $location = $this->session->userdata('dblocation');
        $_data['view'] = 'reports/tabular_details';
        $_data['export_excel'] = $export_excel;
        ...
        $this->Receiving->create_receivings_items_temp_table();
        $headers = $model->getDataColumns();
        $report_data = $model->getData(['start_date' => $start_date, 'end_date' => $end_date, 'employee_id' => $employee_id, 'sale_type' => $sale_type]);
        $summary_data = [];
        $details_data = [];
        foreach ($report_data['summary'] as $key => $row) {
            $summary_data[] = [anchor('sales/receipt/'.$row['sale_id'], 'POS '.$row['sale_id'], ['target' => '_blank']), $row['sale_date'], floor($row['items_purchased']), $row['customer_name'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']), to_currency($row['profit']), $row['payment_type'], $row['comment']];
            foreach ($report_data['details'][$key] as $drow) {
                $details_data[$key][] = [$drow['name'], $drow['category'], $drow['serialnumber'], $drow['description'], floor($drow['quantity_purchased']), to_currency($drow['subtotal']), to_currency($drow['total']), to_currency($drow['tax']), to_currency($drow['profit']), $drow['discount_percent'].'%'];
            }
        }
        ...
        $data = [
            ...
            'details_data'         => $details_data,
            'location'             => ucwords(($location == 'default' ? 'Principal' : $location)),
            'overall_summary_data' => $model->getSummaryData(['start_date' => $start_date, 'end_date' => $end_date, 'employee_id' => $employee_id, 'sale_type' => $sale_type]),
            'export_excel'         => $export_excel,
            'right'                => [2, 4, 5, 6, 7],
            'right_d'              => [4, 5, 6, 7, 8, 9],
        ];
    ...
    public function detailed_sales($start_date, $end_date, $sale_type, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
        ...
        foreach ($locations as $location) {
            ...
            $data = [
                ...
                'location'             => ucwords(($location == 'default' ? 'Principal' : $location)),
                'overall_summary_data' => $model->getSummaryData(['start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type]),
                'export_excel'         => $export_excel,
            ];
            $_data['list'][] = $data;
        }
    ...
    public function detailed_receivings($start_date, $end_date, $sale_type, $location = 'default')
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
        ...
        foreach ($locations as $location) {
            ...
            $details_data = [];
            foreach ($report_data['summary'] as $key => $row) {
                $summary_data[] = [anchor('receivings/receipt/'.$row['receiving_id'], 'RECV '.$row['receiving_id'], ['target' => '_blank']), $row['receiving_date'], $row['items_purchased'], $row['employee_name'], $row['supplier_name'], to_currency($row['total']), $row['payment_type'], $row['comment']];
                foreach ($report_data['details'][$key] as $drow) {
                    $details_data[$key][] = [$drow['name'], $drow['category'], $drow['quantity_purchased'], to_currency($drow['total']), $drow['discount_percent'].'%'];
                }
            }
            $data = [
    ...
    public function inventory_low($export_excel = 0, $location = false)
    {
        $loca = urldecode($this->input->get('loc'));
        if ($loca) {
            $location = $loca;
        ...
        foreach ($locations as $location) {
            ...
            foreach ($report_data as $row) {
                $tabular_data[] = [$row['name'], ($row['item_number'] ? $row['item_number'] : ''), $row['description'], $row['quantity'], $row['reorder_level']];
            }
            $data = [
    ...
    public function inventory_summary($export_excel = 0, $location = 'default')
    {
        $_data['view'] = 'reports/tabular';
        $_data['export_excel'] = $export_excel;
        ...
        foreach ($locations as $location) {
            ...
            $report_data = $model->getData([]);
            foreach ($report_data as $row) {
                $tabular_data[] = [$row['name'], $row['item_number'], $row['description'], floor($row['quantity']), floor($row['reorder_level'])];
            }
            $data = [
    ...
    public function inventory_details_items($export_excel = 0, $location)
    {
    ...
    public function accounts_payable($export_excel = 0, $location = false)
    {
        if (!$location) {
            $location = $this->session->userdata('dblocation');
        }
        ...
        $this->load->model('reports/Detailed_receivings');
        $model = $this->Detailed_receivings;
        $headers = ['', $this->lang->line('reports_date'), $this->lang->line('reports_items_received'), $this->lang->line('employees_employee'), $this->lang->line('suppliers_supplier'), $this->lang->line('reports_accounts_payable_payment'), $this->lang->line('reports_payment_type'), $this->lang->line('reports_accounts_payable_debt')];
        ...
        foreach ($locations as $location) {
            ...
            $this->Receiving->con = $this->Sale->con;
            $this->Receiving->create_receivings_items_temp_table();
            $report_data = $model->getData([], true);
            $report_data_transfer = $this->Transfers->transfers_receivable();
            foreach ($report_data as $row) {
                $tabular_data[] = ['RECV'.$row['receiving_id'], $row['receiving_date'], $row['items_purchased'], $row['employee_name'], $row['supplier_name'], to_currency($row['total']), ($row['payment_type']) ? $row['payment_type'] : $this->lang->line('reports_no_payments'), (to_currency($row['money'] - $row['total']))];
            }
            foreach ($report_data_transfer as $row) {
                ...
                $tabular_data[] = ['TRANS'.$row['receiving_id'], $row['receiving_date'], $row['items_purchased'], $row['employee_name'], $row['supplier_name'], to_currency(array_sum($cadenas)), $row['payment_type'], to_currency($row['total'])];
            }
    ...
    public function accounts_receivable($export_excel = 0, $location = 'default')
    {
        $_data['view'] = 'reports/tabular';
        $_data['export_excel'] = $export_excel;
        $this->load->model('transfers');
        $model = $this->Transfers;
        $headers = ['', $this->lang->line('reports_date'), $this->lang->line('reports_items_received'), $this->lang->line('suppliers_supplier'), $this->lang->line('reports_accounts_payable_payment'), $this->lang->line('reports_payment_type'), $this->lang->line('reports_accounts_receivable_credit')];
        ...
        foreach ($locations as $location) {
            ...
            foreach ($report_data as $row) {
                ...
                $tabular_data[] = [$row['receiving_id'], $row['receiving_date'], $row['items_purchased'], $row['supplier_name'], to_currency($payme), $row['payment_type'], (to_currency($row['total'] - $payme))];
            }
            $data = [
    ...
    public function pending_orders($id_orders = '', $error = false, $location = false)
    {
    ...
    public function control_stock($location = false)
    {
        ...
        //Const
        $data['status'] = [$this->lang->line('orders_status1'),
    ...
    public function see_dialog_error($id_orders = '', $error = false)
    {
        $error = explode(':', $error);
        switch ($error[0]) {
            case 1:
