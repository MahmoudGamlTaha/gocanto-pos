    ...
    */
    ...
    public function save($item_kit_id = -1)
    {
        ...
        if ($this->Item_kit->save($item_kit_data, $item_kit_id)) {
            //New item kit
            if ($item_kit_id == -1) {
                echo json_encode(['success' => true, 'message' => $this->lang->line('item_kits_successful_adding').' '.
                $item_kit_data['name'], 'item_kit_id' => $item_kit_data['item_kit_id'], ]);
                $item_kit_id = $item_kit_data['item_kit_id'];
            } else {
                //previous item

                echo json_encode(['success' => true, 'message' => $this->lang->line('item_kits_successful_updating').' '.
            ...
            echo json_encode(['success' => false, 'message' => $this->lang->line('item_kits_error_adding_updating').' '.
    ...
    public function delete()
    {
        ...
        if ($this->Item_kit->delete_list($item_kits_to_delete)) {
            echo json_encode(['success' => true, 'message' => $this->lang->line('item_kits_successful_deleted').' '.
            count($item_kits_to_delete).' '.$this->lang->line('item_kits_one_or_multiple'), ]);
        } else {
            echo json_encode(['success' => false, 'message' => $this->lang->line('item_kits_cannot_be_deleted')]);
        }
    }
    ...
    public function generate_barcodes($item_kit_ids)
    {
        ...
        foreach ($item_kit_ids as $item_kid_id) {
            ...
            $result[] = ['name' => $item_kit_info->name, 'id' => 'KIT '.$item_kid_id];
        }
